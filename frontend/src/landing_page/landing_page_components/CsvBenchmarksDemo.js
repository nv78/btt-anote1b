...
export default function CsvBenchmarksDemo() {
  ...
  return (
    <div className="min-h-screen bg-black px-4 py-10 text-gray-100">
    <div className="max-w-6xl mx-auto">
      <div className="text-xs uppercase tracking-[0.2em] text-[#EDDC8F] mb-3">Benchmark Runner</div>
      <h1 className="text-3xl sm:text-4xl font-extrabold mb-2 bg-gradient-to-r from-[#EDDC8F] to-[#F1CA57] bg-clip-text text-transparent">CSV Benchmarks</h1>
      ...
      {loading && <div className="mb-3 text-sm text-[#EDDC8F]">Running... This may take a minute for real models.</div>}
      ...
      <div className="mb-6 flex items-center gap-3 flex-wrap border border-gray-800 bg-gray-950 rounded-lg p-4">
        ...
        <input
          ...
          className="border border-gray-700 bg-gray-900 px-2 py-1 w-24 rounded-md text-white focus:outline-none focus:border-[#EDDC8F]/70"
        />
        <label className="ml-4 text-sm flex items-center gap-2">
          <input type="checkbox" checked={echoMode} onChange={e => setEchoMode(e.target.checked)} />
          Echo mode (no API keys)
        ...
        <button
          ...
          className="ml-3 px-4 py-2 bg-[#EDDC8F] hover:bg-[#F1CA57] text-black font-semibold rounded-md disabled:opacity-40"
        >
          Run Benchmarks
        </button>
        <button
          ...
          className="px-4 py-2 border border-[#EDDC8F]/60 text-[#EDDC8F] rounded-md hover:bg-[#EDDC8F]/10 disabled:opacity-40"
        >
          Select All
        </button>
        <button
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <input
          ...
          className="border border-gray-700 bg-gray-900 px-2 py-1 text-sm rounded-md text-white w-56 focus:outline-none focus:border-[#EDDC8F]/70"
        />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-8">
        {filteredDatasets.map(ds => (
          <label key={ds.filename} className="border border-gray-800 rounded-lg p-3 flex items-center gap-3 bg-gray-950 hover:border-[#EDDC8F]/50 cursor-pointer transition-colors">
            <input
              type="checkbox"
      ...
      {!echoMode && modelCatalog.length > 0 && (
        <div className="mb-8">
          <h2 className="font-semibold text-[#EDDC8F] mb-2">Models</h2>
          <p className="text-xs text-gray-400 mb-3">
            Select specific models to run. If none selected, backend defaults are used.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {modelCatalog.map(m => (
              <label key={m.name} className="border border-gray-800 rounded-lg p-3 flex items-center gap-3 bg-gray-950 hover:border-[#EDDC8F]/50 cursor-pointer transition-colors">
                <input
                  type="checkbox"
      ...
      {results?.runs && results.runs.length > 0 && (
        <div className="overflow-x-auto border border-gray-800 rounded-lg bg-gray-950 p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setShowRaw(v => !v)} className="text-sm underline text-[#EDDC8F]">
              {showRaw ? 'Hide' : 'Show'} sample response
            </button>
          </div>
          {showRaw && (
            <pre className="border border-gray-800 rounded-md bg-black text-[#EDDC8F] p-3 text-xs mb-3 overflow-auto max-h-64">
              {rawJson}
            </pre>
          )}
          <table className="min-w-full border border-gray-800 text-sm">
            <thead className="bg-black text-[#EDDC8F]">
              <tr>
